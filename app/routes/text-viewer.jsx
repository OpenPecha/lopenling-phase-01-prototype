...
function createQuestion(username, category_id, topic_name, post_text) {
  const apiObj = new DiscourseApi(
    "https://lopenling.org",
    "69692ed5e365396b9ae17acd63334b444cf07ab1b8c0f43a9e67764fb7a31b02"
  );

  return apiObj.addTopic(username, category_id, topic_name, post_text);
}
...
export async function action({ request }) {
  ...
  let post_text = formData.get("body");
  await createQuestion(user.username, 55, topic_name, post_text);
  return redirect("/text-viewer");
}
export default function () {
  ...
  const selectionSpanStart = parseInt(
    searchParams.get("selectionStart") ?? "0"
  );
  const selectionSpanEnd = parseInt(searchParams.get("selectionEnd") ?? "0");
  if (selectionSpanEnd !== 0) {
    data.text = highlight(data.text, selectionSpanStart, selectionSpanEnd);
  }
  ...
  return (
    <main style={{ width: "60%", margin: "auto" }}>
      ...
      <Form method="post">
        <input placeholder="topic" type="text" name="topic"></input>
